    ...
    if (!stuff) {
      ...
      newStuff.save(function () {
        const expire = new ExpireWeek({
          ...
          .save()
          .then(res.status(201).json({ message: `${req.body.type} insered` }))
      })
    } else {
      ExpireWeek.findOne({ idStuff: stuff._id }).then((expire) => {
        if (!expire) {
          const newStuff = new Library({
            _userId: dataToken.userId,
            id: req.body.id,
...
exports.getUserStuff = async (req, res, next) => {
  ...
  if (token === 'null' || token === 'undefined') {
    ...
    Library.find({ _userId: dataToken.userId }).then((data) => {
      data.forEach((stuff) => {
        ExpireWeek.findOne({ idStuff: stuff._id }).then(async (expire) => {
          if (!expire) {
